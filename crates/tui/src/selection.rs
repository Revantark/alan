//! Reusable mouse text-selection helpers for line-based text views.
//!
//! Positions address display columns (not byte/char indices), so wide CJK
//! glyphs occupy two columns and selection highlights stay visually correct.
//! No clipboard integration: extracting text is the caller's concern.
...
/// Position in a text view: line index and display column offset.
...
/// Applies selection highlight to the rendered viewport of `lines`.
///
/// Takes the viewport by value and highlights in place, so lines outside the
/// selection are never cloned.
pub fn apply_selection_to_lines(
    ...
    for (rel_idx, line) in lines.iter_mut().enumerate() {
        ...
        *line = highlight_line(line, col_start, col_end, selection_bg, selection_fg);
    }
    lines
}
...
fn highlight_line(
    ...
    for span in &line.spans {
        ...
        let span_width: usize = span_text.width();
        let span_end = current_col + span_width;

        // Fast paths: span entirely outside or entirely inside the selection.
...
/// Extracts selected text as a string from the lines, trimming padding if needed.
...
/// Whether `pos` (a display-column point) lies inside `rect`.
pub fn rect_contains(rect: Rect, col: u16, row: u16) -> bool {
    crate::geometry::rect_contains(rect, col, row)
}
...
mod tests {
    ...
    #[test]
    fn highlights_line_multibyte_cjk() {
